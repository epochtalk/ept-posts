...
module.exports = function(opts) {
  ...
  var q = 'SELECT p.id, p.title, p.user_id, u.username, p.thread_id, p.position, p.body, t.board_id, b.name AS board_name FROM posts p, users u, threads t, boards b, to_tsquery(\'simple\', $1) AS q WHERE (p.tsv @@ q) AND (p.user_id = u.id) AND (p.thread_id = t.id) AND (t.board_id = b.id) ORDER BY p.created_at DESC LIMIT $2 OFFSET $3';
